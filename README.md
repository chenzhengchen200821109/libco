...
***
# 简介  
libcox/是一个基于[libco]开发的现代化C++11高性能网络服务器，其目的是使[libco]的使用更加方便。

# 特性

1. 现代版的C++11接口，自动管理资源，无需担心内存泄露。
2. 非阻塞异步接口都是C++11的functional/bind形式的回调仿函数（不是[libco]中的C风格的函数指针）。
3. CPU多核友好和线程安全。
4. 非阻塞纯异步多线程TCP服务器/客户端。
5. 支持多进程模式。
6. 添加Buffer缓冲区类，程序自动处理收发缓冲区。
